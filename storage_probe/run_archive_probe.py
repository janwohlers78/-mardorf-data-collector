...
far_checks={
    ...
    "gefs_384_dpt":dc.sql("SELECT count(*) FROM v WHERE model='GEFS-control' AND lead_hours=384 AND parameter_native='2d'").fetchone()[0],
    "gefs_246_products":dc.sql("SELECT DISTINCT provider_product FROM v WHERE model='GEFS-control' AND lead_hours=246 ORDER BY 1").fetchall(),
    "gefs_384_products":dc.sql("SELECT DISTINCT provider_product FROM v WHERE model='GEFS-control' AND lead_hours=384 ORDER BY 1").fetchall(),
}
dc.close()
...
if far_checks["gefs_246_products"] != [("gefs_0p50a+gefs_0p50b",)]:
    raise SystemExit(f"GEFS f246 product identity mismatch: {far_checks['gefs_246_products']}")
if far_checks["gefs_384_products"] != [("gefs_0p50a+gefs_0p50b",)]:
    raise SystemExit(f"GEFS f384 product identity mismatch: {far_checks['gefs_384_products']}")
...
summary={
    ...
    "source_live_run_id":36244491836,
